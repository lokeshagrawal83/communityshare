...
class Conversation(Base, Serializable):
    ...
    def validate_(self, key, user_id):
        ...
        if user is None:
            ...
            message = 'Cannot create conversation with {username}' \
                      'because they have not confirmed their email'
            raise ValidationException(message.format(username=user.name))
        return user_id
    ...
    def args_to_query(cls, args, requester):
        ...
        messages_date_created_greaterthan = args.get('messages.date_created.greaterthan', None)
        query = None
